...
#else
	if (net_pton(AF_INET6, session->server, (void *) &saddr.sin6_addr) <= 0) {
		memset( &ainfo, 0, sizeof(ainfo));
		ainfo.ai_family = AF_INET6;
		ainfo.ai_socktype = SOCK_STREAM;
		ainfo.ai_protocol = 0;

		if (   getaddrinfo(session->server, portStr, &ainfo, &res)
			|| !res) {
			session->lasterror = LIBIRC_ERR_RESOLV;
			return 1;
		}
...
static void libirc_process_incoming_data (irc_session_t *session,
	...
	// Handle PING/PONG
	if (command && !strncmp (command, "PING", (size_t) max(buf_end - command, 0)) && params[0]) {
		if (!(session->options & LIBIRC_OPTION_IGNORE_PING))
	...
	if (code) {
		...
		if (!strncmp(command, "NICK", (size_t) max(buf_end - command, 0))) {
			/*
		...
		} else if (!strncmp (command, "QUIT", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_quit)
				(*session->callbacks.event_quit)(session, command, prefix, params, paramindex);
		} else if (!strncmp (command, "JOIN", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_join )
				(*session->callbacks.event_join)(session, command, prefix, params, paramindex);
		} else if (!strncmp (command, "PART", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_part)
				(*session->callbacks.event_part)(session, command, prefix, params, paramindex);
		} else if (!strncmp (command, "MODE", (size_t) max(buf_end - command, 0))) {
			if (paramindex > 0 && !strncmp (params[0], session->nick, strlen(session->nick))) {
				params[0] = params[1];
				paramindex = 1;

				if (session->callbacks.event_umode)
					(*session->callbacks.event_umode)(session, "UMODE", prefix, params, paramindex);
			} else {
				if (session->callbacks.event_mode)
					(*session->callbacks.event_mode)(session, "MODE", prefix, params, paramindex);
			}
		} else if (!strncmp(command, "TOPIC", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_topic )
				(*session->callbacks.event_topic) (session, command, prefix, params, paramindex);
		} else if (!strncmp(command, "KICK", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_kick)
				(*session->callbacks.event_kick) (session, command, prefix, params, paramindex);
		} else if (!strncmp(command, "ERROR", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_error)
				(*session->callbacks.event_error) (session, command, prefix, params, paramindex);
		} else if (!strncmp(command, "PRIVMSG", (size_t) max(buf_end - command, 0))) {
			if (paramindex > 1) {
				...
				if (params[1][0] == 0x01 && params[1][msglen-1] == 0x01) {
					...
					if (!strncasecmp(ctcp_buf, "DCC ", 4)) {
						...
						(*session->callbacks.event_ctcp_action)(session, "CTCP_ACTION", prefix, params, paramindex);
					} else {
						params[0] = ctcp_buf;
						paramindex = 1;

						if (session->callbacks.event_ctcp_req)
							(*session->callbacks.event_ctcp_req)(session, "CTCP_REQ", prefix, params, paramindex);
					}
					...
					if (session->callbacks.event_channel)
						(*session->callbacks.event_channel)(session, "CHANMSG", prefix, params, paramindex);
				} else {
					if (session->callbacks.event_server_msg)
						(*session->callbacks.event_server_msg)(session, "SERVMSG", prefix, params, paramindex);
				}
			}
		} else if (!strncmp (command, "NOTICE", (size_t) max(buf_end - command, 0))) {
			size_t msglen = strlen (params[1]);
			...
			if (paramindex > 1 && params[1][0] == 0x01 && params[1][msglen-1] == 0x01) {
				...
				if (session->callbacks.event_ctcp_rep)
					(*session->callbacks.event_ctcp_rep)(session, "CTCP_REPL", prefix, params, paramindex);
			} else if (!strncasecmp (params[0], session->nick, strlen(session->nick))) {
				if (session->callbacks.event_notice)
					(*session->callbacks.event_notice)(session, "PRIVNOTICE", prefix, params, paramindex);
			} else if (   params[0][0] == '#'
					   || params[0][0] == '&'
					   || params[0][0] == '!'
					   || params[0][0] == '+') {
				...
				if (session->callbacks.event_channel_notice)
					(*session->callbacks.event_channel_notice)(session, "CHANNOTICE", prefix, params, paramindex);
			} else {
				if (session->callbacks.event_server_notice)
					(*session->callbacks.event_server_notice)(session, "SERVNOTICE", prefix, params, paramindex);
			}
		} else if (!strncmp(command, "INVITE", (size_t) max(buf_end - command, 0))) {
			if (session->callbacks.event_invite)
				(*session->callbacks.event_invite) (session, command, prefix, params, paramindex);
		} else if (!strncmp(command, "KILL", (size_t) max(buf_end - command, 0))) {
...
int irc_process_select_descriptors (irc_session_t *session,
	...
	if (FD_ISSET (session->sock, in_set)) {
		size_t offset;
		ssize_t length = session_socket_read(session);
		...
		session->incoming_offset += (size_t) length;
	...
	if (FD_ISSET(session->sock, out_set)) {
		...
		if (length > 0 && session->outgoing_offset - (size_t) length > 0)
			memmove(session->outgoing_buf, session->outgoing_buf + length, session->outgoing_offset - (size_t) length);

		session->outgoing_offset -= (size_t) length;
		libirc_mutex_unlock(&session->mutex_session);
	}
...
int irc_cmd_quit (irc_session_t *session,
				  const char *reason) {
	return irc_send_raw(session, "QUIT :%s", reason ?: "quit");
}
